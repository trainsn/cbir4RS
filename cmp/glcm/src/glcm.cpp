...
void GLCM::CalcuTextureEValue(Mat src, TextureEValues& EValue, int size, GrayLevel level)
{
    ...
    EValue.contrastMean = 0; EValue.energyMean = 0; EValue.entropyMean = 0; EValue.homogenityMean = 0;
    EValue.contrastStd = 0; EValue.energyStd = 0; EValue.entropyStd = 0; EValue.homogenityStd = 0;
    ...
    std::vector<float> temp_contrast;
    std::vector<float> temp_homogenity;
    std::vector<float> temp_entropy;
    int imgSize = imgGray.rows * imgGray.cols;
    ...
    for(int i = 0; i < imgGray.rows; i++)
    {
        for(int j = 0; j < imgGray.cols; j++)
        {
            ...
            EValue.contrastMean += contrast / imgSize;
            EValue.energyMean += energy / imgSize;
            EValue.entropyMean += entropy / imgSize;
            EValue.homogenityMean += homogenity / imgSize;
            temp_contrast.push_back(contrast);
            temp_energy.push_back(energy);
            temp_entropy.push_back(entropy);
            temp_homogenity.push_back(homogenity);
        }
    }
    for (int i = 0; i < imgSize; i++)
    {
        EValue.contrastStd += (temp_contrast[i] - EValue.contrastMean) * (temp_contrast[i] - EValue.contrastMean) / imgSize;
        EValue.energyStd += (temp_energy[i] - EValue.energyMean) * (temp_energy[i] - EValue.energyMean) / imgSize;
        EValue.entropyStd += (temp_entropy[i] - EValue.entropyMean) * (temp_entropy[i] - EValue.entropyMean) / imgSize;
        EValue.homogenityStd += (temp_homogenity[i] - EValue.homogenityMean) * (temp_homogenity[i] - EValue.homogenityMean) / imgSize;
    }
    EValue.contrastStd = sqrt(EValue.contrastStd);
    EValue.energyStd = sqrt(EValue.energyStd);
    EValue.entropyStd  = sqrt(EValue.entropyStd);
    EValue.homogenityStd = sqrt(EValue.homogenityStd);
}
